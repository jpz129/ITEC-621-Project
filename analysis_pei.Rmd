...
data$Division <- as.numeric(as.factor(data$Division))
# Regions:
# 1 = Midwest
# 2 = Northeast
# 3 = South
...
X<-data[,-3] # remove date (non-numeric)
names(X)
M<-cor(X)
library(corrplot)
...
```
```{r}
plot(data$median_listing_price~.,data=data)
```
## log transform
```{r}
names(data)
data[,-c(3,10)] %>% 
  ...
  data_log
data_log <- as.data.frame(Map(function(x) replace(x, is.infinite(x), NA), data_log))
...
summary(lm.fit) # why pop_density_sqmi_log NA????
```
seems to keep all var so far

## classification regression
```{r}
data_log$median_listing_price_log_clas <- data_log$median_listing_price_log
# data_log$median_listing_price_log <- data_log$median_listing_price_log
mlp_log_mean <- mean(data_log$median_listing_price_log_clas)
mlp_log_std <- sd(data_log$median_listing_price_log_clas)
ifelse(data_log$median_listing_price_log_clas > mlp_log_mean+mlp_log_std, "High",
       ifelse(data_log$median_listing_price_log_clas > mlp_log_mean+mlp_log_std,"Low","Median")) ->
  data_log$median_listing_price_log_clas
data_log$median_listing_price_log_clas <- as.numeric(as.factor(data_log$median_listing_price_log_clas))
head(data_log)

lm.clas.fit<-lm(median_listing_price_log_clas~.-median_listing_price_log,data=data_log)
summary(lm.clas.fit)
```
seems regression is better
